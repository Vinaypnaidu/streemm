# apps/api/alembic/versions/6b5b3b8e03a4_fresh_init.py
"""fresh init
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
            name=op.f("fk_videos_user_id_users"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_videos")),
    )
    op.create_index(
        "ix_videos_user_id_created_at",
        "videos",
        ["user_id", "created_at"],
        unique=False,
    )
    op.create_table(
        ...
        sa.UniqueConstraint(
            "video_id", "kind", "label", name=op.f("uq_video_assets_video_id")
        ),
    )
    op.create_index(
        "ix_video_assets_video_id", "video_assets", ["video_id"], unique=False
    )
    op.create_table(
        "watch_history",
        sa.Column("user_id", sa.UUID(), nullable=False),
        sa.Column("video_id", sa.UUID(), nullable=False),
        sa.Column(
            "last_position_seconds", sa.Float(), server_default="0", nullable=False
        ),
        sa.Column(
            "last_watched_at",
            sa.DateTime(timezone=True),
        ...
        sa.ForeignKeyConstraint(
            ["video_id"],
            ["videos.id"],
            name=op.f("fk_watch_history_video_id_videos"),
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("user_id", "video_id", name=op.f("pk_watch_history")),
    )
    op.create_index(
        "ix_watch_history_user_lastwatched",
        "watch_history",
