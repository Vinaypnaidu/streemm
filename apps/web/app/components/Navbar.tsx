      ...
      <nav className="sticky top-0 z-40 w-full border-b border-neutral-800 bg-neutral-900/95 backdrop-blur">
        <div className="px-4 h-14 flex items-center gap-4 text-neutral-100">
          ...
          <div className="flex items-center gap-3">
            <button className="p-2 rounded-full hover:bg-neutral-800" aria-label="Menu">
          ...
          <div className="ml-auto flex items-center gap-2 relative" ref={menuRef}>
            ...
            {showMenu && (
              <div role="menu" className="absolute right-0 top-12 w-48 rounded-lg border border-neutral-800 bg-neutral-900 shadow-lg">
                <Link href="/upload" className="block px-4 py-2 text-sm hover:bg-neutral-800" onClick={() => setShowMenu(false)}>Upload video</Link>
                <Link href="/videos" className="block px-4 py-2 text-sm hover:bg-neutral-800" onClick={() => setShowMenu(false)}>Your videos</Link>
              </div>
            )}
            <button onClick={logout} className="rounded-full bg-neutral-100 text-neutral-900 px-4 py-2 text-sm font-medium hover:opacity-90">
              Logout
            </button>
          </div>
        </div>
